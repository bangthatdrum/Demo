...
const Navbar = () => {
  ...
  const feeBalances = useSelector((state) => state.exchange.feeBalances);
  ...
  return (
    <div className="exchange__header grid">
      ...
      <div className="exchange__header--networks flex">
        ...
        {chainId && (
          <select
            name="networks"
            id="networks"
            value={chainId ? `0x${chainId.toString(6)}` : `0`}
      ...
      <div className="exchange__header--account flex">
        {balance ? (
          <p>
            <small>{Number(balance).toFixed(4)} ETH</small>
          </p>
        ) : (
          <p>
            <small>{balance}</small>0 ETH
          </p>
        )}
